...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        proceed = scheme_eval(first, env)
        check_procedure(proceed)
        return proceed.eval_call(rest, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) is not len(vals):
            raise SchemeError("Not my error")
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        "*** YOUR CODE HERE ***"
        no_eval = scheme_apply(self, operands, env)
        yes_eval = Thunk(complete_eval(no_eval), env)
        return yes_eval
        # END Problem 21
...
def do_and_form(expressions, env):
    ...
    while True:
        ...
        sec_one = scheme_eval(expressions.first, env)
        if scheme_falsep(sec_one):
            return sec_one
        expressions = expressions.second
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    values = nil
    while bindings != nil:
        curr_expression = bindings.first
        check_form(bindings.first, 2, 2)
        formals = Pair(curr_expression.first, formals)
        values = Pair(scheme_eval(curr_expression.second.first, env), values)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            "*** YOUR CODE HERE ***"
            proceed = scheme_optimized_eval(first, env)
            check_procedure(proceed)
            result = proceed.eval_call(rest, env)
            # END PROBLEM 20
    return result
